...
function Invoke-Git {
    ...
    # Use System.Diagnostics.Process directly. This avoids:
    # 1. Windows PowerShell converting Git stderr warnings into terminating errors.
    # 2. Start-Process re-parsing/mangling Git format arguments such as %ad and %an.
...
function Get-UniqueCommitData {
    ...
    $result = Invoke-Git -Arguments @(
        "log",
        "--pretty=format:%H`t%aI`t%an`t%ae`t%s",
        "$TargetRef..$SourceRef"
    )
